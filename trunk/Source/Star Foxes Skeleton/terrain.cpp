...
void Terrain::setupBuildings()
{
	for(int i = 0; i < 1; i++)
	{
		...
		D3DXCreateBox(g_pDevice, 1, 1, 1, &buildingmesh[14], NULL);
		...
		D3DXCreateBox(g_pDevice, 1, 1, 1, &buildingmesh[15], NULL);
		...
		D3DXCreateBox(g_pDevice, 1, 1, 1, &buildingmesh[16], NULL);
